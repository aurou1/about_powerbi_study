...
数据由脚本自动生成，共 **15,780 行**（维度 1,806 + 事实 11,974），埋好了可用于练习的真实差异：地区销售额差异、品类毛利率差异、客户等级消费分层、月度季节性（618 / 双11 / 双12 冲高）。
...
> 参考资料层是「字典和题目」，教程层是「为什么 + 怎么做 + 什么时候用」。两条教程线用同一套数据：Power BI 负责把数据变成报表，SQL 负责把数据查出来算明白，通过相对路径互链，内容不重复。
