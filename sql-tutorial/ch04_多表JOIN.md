...
### 3.2 三种 JOIN 用集合图一次看懂

把「A、B 两表的行」想成两个集合，JOIN 就是按连接键把它们配对：

```
A = dim_customer(全部500客户)      B = fact_order(全部3529订单)

INNER JOIN = 交集: 只留下"有订单的客户+订单"
   A ∩ B = 有订单的客户(498) 匹配上的订单

LEFT JOIN  = A ∪ (A与B的匹配): 所有客户都在
             没订单的客户 → 订单列补 NULL(2个0单客户, ch05会查)
  图形:
  ┌─────────────┐
  │ A(客户)      │
  │  ┌────────┐  │
  ...
  │  └────────┘  │
  └─────────────┘
  LEFT JOIN 把整个 A 圈都保留, 只有"配对成功"的才连上 B 的数据

RIGHT JOIN = 反过来保 B
FULL OUTER JOIN = 并集: MySQL 不直接支持(用 LEFT UNION RIGHT 模拟)
```

> 心智模型：**INNER 只留配对成功的行；LEFT 无条件保左表全部，右表没配上就补 NULL。** ch05 练习里「找 0 单客户」靠的就是 LEFT JOIN 后 `WHERE 右表主键 IS NULL`。

### 3.3 JOIN 在数据库里到底怎么执行？（连接算法）

`JOIN` 不是魔法，MySQL 要把两边的行"配起来"，底层有三种算法，**驱动表选错会慢 100 倍**：

```
① 嵌套循环连接 NLJ（小表驱动大表, 最常用）
   对 A 每一行, 去 B 里找匹配:
   A行1 → 扫B找匹配  A行2 → 扫B找匹配 ...
   若 B 的连接键有索引 → 每次查找 O(log n) → 总 O(A行数 × log B)
   若 B 没索引       → 每次全表扫     → 总 O(A行数 × B行数) ← 灾难

② 块嵌套循环 BNLJ（B 没索引时的优化: 把 A 攒一批一起比, 减少扫B次数）
   MySQL 5.7 及以后对"无索引连接"自动用

③ 哈希连接 HJ（MySQL 8.0 新增, 等值连接最快）
   先把小表按连接键建成内存哈希表(查询时一次性), 再让大表每行 O(1) 查找
   → 两边都没索引的大表连接, 哈希连接远快于嵌套循环

图形(驱动表=外层循环):
  驱动表(小)  被驱动表(大, 有索引)
   ┌────┐        ┌────────────┐
   │ 10 │──查找─▶│ 索引找到=1行│   ← 内层走索引, 快
   │ 20 │──查找─▶│ 索引找到=1行│
   └────┘        └────────────┘
```

**驱动表是谁？** MySQL 优化器自动选——通常是**行数少的表当驱动表**（外层循环次数少）。你控制不了它，但能帮它：**给被驱动表的连接键建索引**（ch10 会讲），并**用 WHERE 尽早过滤**让小表更小。

> 为什么这条对你有用？你写 JOIN 慢查询时，第一个反应不该是"换 LEFT/RIGHT"，而是 `EXPLAIN` 看**驱动表、连接类型、有没有走索引**（ch10 一整章教这个）。小表驱动大表 + 索引，是 JOIN 快的前提。
